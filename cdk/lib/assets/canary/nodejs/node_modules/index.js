...
const apiCanaryBlueprint = async function () {
    ...
    try {
        // ステップ1: APIエンドポイントを呼び出す (/rolldice?rolls=12)
        ...
        // ステップ2: ヘルスチェックエンドポイントを呼び出す
        const healthCheckUrl = `${url}/healthcheck`;
        log.info(`Testing healthcheck endpoint: ${healthCheckUrl}`);
        let healthResponse = await synthetics.executeHttpStep('Healthcheck Test', healthCheckUrl, requestOptions);
        
        if (healthResponse.statusCode !== 200) {
            throw new Error(`Healthcheck failed. Status code: ${healthResponse.statusCode}`);
        }
